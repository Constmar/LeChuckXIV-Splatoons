...
function sell()
	...
	yield("/callback MJIDisposeShopShipping true 11 " .. (GetItemCount(37554) - minItems))
	yield("/wait 0.5")
	yield("/callback SelectYesno true 0")
	yield("/wait 1.25")
	--iron
	yield("/callback MJIDisposeShop true 12 24 <wait.0.5>")
	yield("/callback MJIDisposeShopShipping true 11 " .. (GetItemCount(37572) - minItems))
	yield("/wait 0.5")
	yield("/callback SelectYesno true 0")
	yield("/wait 1.25")
	--quartz
	yield("/callback MJIDisposeShop true 12 25 <wait.0.5>")
	yield("/callback MJIDisposeShopShipping true 11 " .. (GetItemCount(37573) - minItems))
	yield("/wait 0.5")
	yield("/callback SelectYesno true 0")
	yield("/wait 1.25")
	--leucogranite
	yield("/callback MJIDisposeShop true 12 26 <wait.0.5>")
	yield("/callback MJIDisposeShopShipping true 11 " .. (GetItemCount(37574) - minItems))
	yield("/wait 0.5")
	yield("/callback SelectYesno true 0")
	yield("/wait 1.25")
...
::start::
if GetItemCount(37554) >= maxItems then 
	sell()
	if not GetCharacterCondition(4) then yield('/gaction "mount roulette"') end
	while not GetCharacterCondition(4) do yield("/wait 0.1") end
